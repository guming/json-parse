from typing import Dict, List, Mapping, Callable, Union
from typeguard import typechecked

from exception import JsonQLReferenceError
...
def find_in_stack(stack: Stack, name: str, absolute: bool) -> RuntimeValue:
    ...
    for frame in reversed(stack):
        if name in frame:
            return frame[name]
    raise JsonQLReferenceError(f"Could not find {name} in stack")